...
class WhenTradeLogger:
    ...
    def _load_default_config(self) -> Dict[str, Any]:
        ...
        # 从环境变量获取配置
        log_level = os.getenv('WHENTRADE_LOG_LEVEL', 'INFO').upper()
        log_dir = os.getenv('WHENTRADE_LOG_DIR', './logs')
    ...
    def _add_structured_handler(self, logger: logging.Logger):
        ...
        log_file = log_dir / 'core_structured.log'
    ...
    def log_analysis_start(self, logger: logging.Logger, stock_symbol: str, analysis_type: str, session_id: str):
        """记录分析开始"""
        logger.info(
            f"🚀 开始分析 - 标的: {stock_symbol}, 类型: {analysis_type}",
            extra={
    ...
    def log_analysis_complete(self, logger: logging.Logger, stock_symbol: str, analysis_type: str,
        ...
        """记录分析完成"""
        logger.info(
            f"✅ 分析完成 - 标的: {stock_symbol}, 耗时: {duration:.2f}s, 成本: ¥{cost:.4f}",
            extra={
                'stock_symbol': stock_symbol,
                'analysis_type': analysis_type,
                'session_id': session_id,
    ...
    def log_module_start(self, logger: logging.Logger, module_name: str, stock_symbol: str,
        ...
        """记录模块开始分析"""
        logger.info(
            f"📊 [模块开始] {module_name} - 标的: {stock_symbol}",
            extra={
    ...
    def log_module_complete(self, logger: logging.Logger, module_name: str, stock_symbol: str,
        ...
        logger.info(
            f"📊 [模块完成] {module_name} - {status} - 标的: {stock_symbol}, 耗时: {duration:.2f}s",
            extra={
                'module_name': module_name,
                'stock_symbol': stock_symbol,
                'session_id': session_id,
    ...
    def log_module_error(self, logger: logging.Logger, module_name: str, stock_symbol: str,
        ...
        """记录模块分析错误"""
        logger.error(
            f"❌ [模块错误] {module_name} - 标的: {stock_symbol}, 耗时: {duration:.2f}s, 错误: {error}",
            extra={
                'module_name': module_name,
                'stock_symbol': stock_symbol,
                'session_id': session_id,
...
# 全局日志管理器实例
_logger_manager: Optional[WhenTradeLogger] = None


def get_logger_manager() -> WhenTradeLogger:
    """获取全局日志管理器实例"""
    global _logger_manager
    if _logger_manager is None:
        _logger_manager = WhenTradeLogger()
...
def setup_logging(config: Optional[Dict[str, Any]] = None):
    """设置项目日志系统（便捷函数）"""
    global _logger_manager
    _logger_manager = WhenTradeLogger(config)
    return _logger_manager
