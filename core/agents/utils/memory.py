...
class FinancialSituationMemory:
    ...
    def _get_local_hash_embedding(self, text):
        """使用本地哈希算法生成嵌入向量，避免外部API依赖"""
        import hashlib
        ...
        try:
            ...
            # 将64个十六进制字符转换为1024维向量
            embedding = []
            
            # 每个十六进制字符可以产生16个浮点数值
            for char in hash_hex:
                base_value = int(char, 16) / 15.0  # 归一化到[0,1]
                
                # 使用不同的数学变换生成16个相关但不同的值
                for i in range(16):
                    transformed = (base_value + i * 0.0625) % 1.0  # 0.0625 = 1/16
                    embedding.append(transformed)
            
            # 验证维度
            if len(embedding) != 1024:
                logger.debug(f"📊 本地嵌入维度: {len(embedding)}，目标: 1024")
                # 填充或截断到1024维
                if len(embedding) < 1024:
            ...
            logger.debug(f"✅ 本地哈希嵌入生成成功，维度: {len(embedding)}")
            return embedding
            
        except Exception as e:
            logger.warning(f"⚠️ 本地嵌入生成失败: {e}，返回随机向量")
            # 降级方案：返回基于文本长度的简单向量
            import random
            random.seed(hash(text))  # 使用文本哈希作为随机种子，保证一致性
            return [random.random() for _ in range(1024)]

    def get_embedding(self, text):
        """Get embedding for a text using local hash-based approach"""
