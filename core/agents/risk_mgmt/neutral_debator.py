...
def create_neutral_debator(llm):
    def neutral_node(state) -> dict:
        ...
        prompt_config = prompt_loader.load_prompt("neutral_debator", language=language)
        
        # 获取系统消息模板
        system_message_template = prompt_config.get("system_message", "您是中性风险分析师。")
        ...
        # 记录提示词版本
        prompt_version = prompt_loader.get_prompt_version("neutral_debator")
        logger.debug(f"⚖️ [中性风险分析师] 使用提示词版本: {prompt_version}")
        ...
        logger.info(f"🌍 [neutral_debator] 语言设置: {language} -> {language_name}")
        logger.debug(f"🔴 [neutral_debator] 语言前缀: {language_prefix}")
        
        # 在prompt前添加语言强制前缀
        enhanced_prompt = language_prefix + prompt
        logger.info(f"✅ [neutral_debator] 已添加语言前缀到prompt")
        ...
        # 中立分析师不应该完全偏向任何一方
        if argument.count("激进") > 5 or argument.count("保守") > 5:
            logger.warning("⚠️ 中立分析师过度偏向某一方，重新生成...")
            # 如果检测到角色混淆，使用默认回应
            argument = "我认为应该采取平衡策略。虽然市场存在增长潜力，但短期风险不容忽视。建议分批建仓，设置合理止损，在控制风险的前提下把握机会。"
        ...
        new_risk_debate_state = {
            ...
            "current_neutral_response": argument,  # Keep for compatibility
